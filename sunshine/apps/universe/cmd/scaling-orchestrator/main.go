...
func (so *ScalingOrchestrator) ExecuteScaling(ctx context.Context, strategyName string) error {
	...
	var funnelsPerDomain int
	if exists {
		targetDomains = append([]string(nil), strategy.TargetDomains...)
		serverIPs = append([]string(nil), strategy.ServerIPs...)
		funnelsPerDomain = strategy.FunnelsPerDomain
	}
	so.mu.RUnlock()
	...
	if len(serverIPs) == 0 {
		return fmt.Errorf("strategy %s has no server IPs configured", strategyName)
	}
	...
	for domainIdx, domain := range targetDomains {
		...
		for funnelNum := 1; funnelNum <= funnelsPerDomain; funnelNum++ {
			wg.Add(1)
			...
			}(domain, funnelNum, domainIdx, serverIPs[(funnelNum-1)%len(serverIPs)])
		}
	}
...
func (so *ScalingOrchestrator) HealthCheck(ctx context.Context) map[string]interface{} {
	...
	so.mu.RLock()
	strategiesCount := len(so.strategies)
	so.mu.RUnlock()
