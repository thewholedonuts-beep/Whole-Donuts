...
func (nm *NetworkManager) DeployNetworkFunnel(ctx context.Context, networkName, funnelID, funnelName, domainFilter, serverIP string) error {
	...
	if exists {
		domainList = append([]string(nil), network.Domains...)
	}
	nm.mu.RUnlock()
	...
	errorChan := make(chan error, len(domainList))
	var mu sync.Mutex
	successCount := 0
	...
	nm.mu.Lock()
	if currentNetwork, ok := nm.networks[networkName]; ok {
		currentNetwork.Funnels = append(currentNetwork.Funnels, funnelID)
	}
	nm.mu.Unlock()

	fmt.Printf("✅ Funnel deployment complete (%d/%d domains)\n", successCount, len(domainList))
	return nil
}
...
func (nm *NetworkManager) ActivateNetwork(networkName string) error {
	...
	if exists {
		funnelIDs = append([]string(nil), network.Funnels...)
	}
	nm.mu.RUnlock()
	...
	for _, funnelID := range funnelIDs {
...
func (nm *NetworkManager) DeactivateNetwork(networkName string) error {
	...
	nm.mu.Lock()
	if currentNetwork, ok := nm.networks[networkName]; ok {
		currentNetwork.Status = "inactive"
	}
	nm.mu.Unlock()
...
func (nm *NetworkManager) ListNetworks() map[string]*NetworkDef {
	...
	for key, network := range nm.networks {
		networks[key] = cloneNetwork(network)
	}
...
func (nm *NetworkManager) GetNetworkStats(networkName string) map[string]interface{} {
	...
	var domainsCount, funnelsCount int
	if exists {
		name = network.Name
		status = network.Status
		description = network.Description
		domainsCount = len(network.Domains)
		funnelsCount = len(network.Funnels)
	}
	nm.mu.RUnlock()
...
func cloneNetwork(network *NetworkDef) *NetworkDef {
	if network == nil {
		return nil
	}

	cloned := *network
	cloned.Domains = append([]string(nil), network.Domains...)
	cloned.Funnels = append([]string(nil), network.Funnels...)
	return &cloned
}
