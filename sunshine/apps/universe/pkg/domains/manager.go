...
func (m *Manager) ListDomains() map[string]*Domain {
	...
	for key, domain := range m.domains {
		domains[key] = cloneDomain(domain)
	}
...
func (m *Manager) ListFunnels() map[string]*Funnel {
	...
	funnels := make(map[string]*Funnel, len(m.funnels))
	for key, funnel := range m.funnels {
		funnels[key] = cloneFunnel(funnel)
	}
...
func cloneDomain(domain *Domain) *Domain {
	...
	cloned := *domain
	cloned.Records = append([]porkbun.Record(nil), domain.Records...)
	cloned.Funnels = append([]string(nil), domain.Funnels...)
	return &cloned
}

func cloneFunnel(funnel *Funnel) *Funnel {
	if funnel == nil {
		return nil
	}

	cloned := *funnel
	if funnel.Config != nil {
		cloned.Config = make(map[string]interface{}, len(funnel.Config))
		for key, value := range funnel.Config {
